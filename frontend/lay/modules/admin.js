layui
  ...
  .define(['conf', 'view', 'api', 'jquery'], function(exports) {
    ...
    //根据当前加载的URL高亮左侧导航
    self.navFocus = function(url) {
      url = url || self.route.href

      var header = $('#app-sidebar')
          .find('[lay-href="/' + url.split('/')[1] + '"]')
          .eq(0)
      if (header.length > 0) {
        header.parents('.layui-nav-item').addClass('layui-nav-itemed')
        header.click()
      }
      ...
      var sidebar = $('#app-sidebar')
        ...
        .eq(0)
      if (sidebar.length > 0) {
        sidebar.parents('.layui-nav-item').addClass('layui-nav-itemed')
        sidebar.click()
      }
    }
